...
const statusLabels: Record<string, string> = {
  NEW: "Новый", REGISTERED: "Зарегистрирован", IN_WORK: "В работе", WAITING: "Ожидание", COMPLETED: "Завершён", ISSUED: "Выдан", CLOSED: "Закрыт",
};

const statusColors: Record<string, "default" | "primary" | "warning" | "success"> = {
  NEW: "default", REGISTERED: "primary", IN_WORK: "warning", WAITING: "warning", COMPLETED: "success", ISSUED: "success", CLOSED: "default",
};
      ...
      <CardContent>
        <Stack spacing={3}>
          <Stack direction="row" spacing={2} sx={{ alignItems: "flex-start", justifyContent: "space-between" }}>
            ...
            <Chip color={statusColors[order.status] ?? "default"} label={statusLabels[order.status] ?? order.status} />
          </Stack>
          <Divider />
          <Stack spacing={2}>
            <DetailRow label="Клиент" value={customer?.name ?? "—"} />
            <DetailRow label="Получен" value={formatDate(order.receivedAt)} />
            <DetailRow label="Планируемая выдача" value={formatDate(order.plannedIssueAt)} />
            <DetailRow label="Выдан" value={formatDate(order.issuedAt)} />
          </Stack>
          <Divider />
          <Stack spacing={0.5}>
            <Typography color="text.secondary" variant="body2">Комментарий</Typography>
            <Typography variant="body1">{order.comment || "—"}</Typography>
          </Stack>
          <Divider />
          {deleteOrderItemMutation.isError && <Alert severity="error">Не удалось удалить позицию заказа.</Alert>}
          <Stack direction="row" spacing={2} sx={{ alignItems: "flex-start", justifyContent: "space-between" }}>
            <OrderItems
              orderId={order.id}
              items={order.items}
