...
### 4.1 Поиск структуры проекта

Для поиска структуры и связей между файлами при наличии локального checkout сначала использовать локальные shell-команды (`find`, `grep`, `sed`, `git`, и аналогичные). Это быстрее и точнее для навигации по фактическому дереву проекта.

GitHub использовать как source of truth для актуального содержимого и истории, а также для чтения большой документации, которую неудобно передавать через чат.

Не просить пользователя копировать большие Markdown-документы в чат, если их можно прочитать из GitHub.
