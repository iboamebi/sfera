...
Для заказа используются общие временные поля `BaseModel`:

```text
created_at → когда запись заказа создана
updated_at → когда запись заказа последний раз изменена
```

Они уже хранятся в persistence model и теперь входят в read-контракт заказа.

API `OrderRead` передаёт:

- `created_at` — дата и время создания заказа;
- `updated_at` — дата и время последнего изменения записи заказа.

Frontend отображает их в карточке заказа как:
...
Это не является полноценным Audit Trail: `updated_at` показывает время последнего изменения записи, но не отвечает на вопросы кто, что и почему изменил. Полный Audit Trail регулируется отдельным архитектурным решением.
