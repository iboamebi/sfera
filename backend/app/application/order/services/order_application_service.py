...
class OrderApplicationService:
    ...
    def __init__(self, repository: OrderRepository, unit_of_work: UnitOfWork, event_dispatcher: object | None = None) -> None:
    ...
    def create(self, command: CreateOrderCommand, user: User) -> Order:
        ...
        with self._uow:
            order = Order.create(id=uuid4(), number=OrderNumber(command.number), customer_id=command.customer_id, received_at=datetime.now(UTC), planned_issue_at=command.planned_issue_at, comment=command.comment)
    ...
    def assign_item_instrument(self, order_id: UUID, item_id: UUID, instrument_id: UUID, user: User) -> Order:
        """Assign a concrete instrument to a type-only order item."""
        require_role(user, UserRole.OPERATOR, UserRole.ADMIN)
        with self._uow:
